...
export function EditUserDialog({open, setIsEditDialogOpen, user}) {
  const submit = async (e) => {
    ...
    try {
      await axios.post("/api/edit-user", {id, name, email});
    ...
    <Dialog open={open} onOpenChange={setIsEditDialogOpen} >
      <DialogContent className="sm:max-w-[425px] bg-white">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            name
            <Input id="name" defaultValue={user?.name} className="col-span-3" type="text" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            email
            <Input id="email" defaultValue={user?.email} className="col-span-3" type="text" />
          </div>
        </div>
        <DialogFooter>
