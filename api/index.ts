...
function isVerified(req: Request, res: Response, next: any) {
  // @ts-ignore
...
app.post('/api/login', passport.authenticate('local', {failureMessage: 'failed'}), function (req, res) {
...
app.get('/api/user', async function (req: Request, res: Response) {
  if (!req.user) {
